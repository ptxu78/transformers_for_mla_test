...
def run_test(batch_size, input_len, output_len, config, Attention):
    ...
    with torch.no_grad():
        for step in range(output_len):
            ...
            # print(f"Step {step+1}, past_key_values' size: {past_key_values.__len__}")
            ...
            if (step + 1) % 10 == 0:
                for layer_idx in range(len(past_key_values)):
                    key_size = past_key_values.key_cache[layer_idx].numel() if past_key_values.key_cache[layer_idx] != [] else 0
                    value_size = past_key_values.value_cache[layer_idx].numel() if past_key_values.value_cache[layer_idx] != [] else 0
                    key_size_gb = key_size / (1024 ** 3)
                    value_size_gb = value_size / (1024 ** 3)
                    print("step:",step)
                    print(f"Layer {layer_idx}: keys={key_size_gb:.4f} GB, values={value_size_gb:.4f} GB")
                    print(f"     key shape = {past_key_values.key_cache[layer_idx].shape}, value shape = {past_key_values.key_cache[layer_idx].shape}")
